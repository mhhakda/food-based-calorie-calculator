# Create the meal.json database for sample prepared meals
import json

meal_data = [
    {
        "id": "meal-001",
        "name": "Vegetable Chicken Bowl",
        "calories": 420,
        "protein": 32.5,
        "carbs": 38.2,
        "fat": 15.8,
        "fiber": 6.2,
        "sugar": 8.5,
        "serving_size": "350 g",
        "serving_grams": 350,
        "source": "Meal Planner",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "meal-002",
        "name": "Indian Dal Rice Combo",
        "calories": 285,
        "protein": 12.8,
        "carbs": 52.6,
        "fat": 3.2,
        "fiber": 7.5,
        "sugar": 2.8,
        "serving_size": "300 g",
        "serving_grams": 300,
        "source": "Meal Planner",
        "origin": "India",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "meal-003",
        "name": "Greek Salad with Feta",
        "calories": 245,
        "protein": 12.5,
        "carbs": 18.2,
        "fat": 16.8,
        "fiber": 5.8,
        "sugar": 12.5,
        "serving_size": "250 g",
        "serving_grams": 250,
        "source": "Meal Planner",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "meal-004",
        "name": "Paneer Butter Masala with Roti",
        "calories": 485,
        "protein": 22.8,
        "carbs": 42.5,
        "fat": 26.2,
        "fiber": 6.8,
        "sugar": 8.5,
        "serving_size": "320 g",
        "serving_grams": 320,
        "source": "Meal Planner",
        "origin": "India",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "meal-005",
        "name": "Grilled Salmon with Quinoa",
        "calories": 385,
        "protein": 28.5,
        "carbs": 32.8,
        "fat": 16.5,
        "fiber": 4.2,
        "sugar": 2.8,
        "serving_size": "280 g",
        "serving_grams": 280,
        "source": "Meal Planner",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "meal-006",
        "name": "Masala Dosa with Sambar",
        "calories": 295,
        "protein": 8.2,
        "carbs": 48.5,
        "fat": 8.5,
        "fiber": 4.8,
        "sugar": 6.2,
        "serving_size": "200 g",
        "serving_grams": 200,
        "source": "Meal Planner",
        "origin": "India",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "meal-007",
        "name": "Mediterranean Hummus Bowl",
        "calories": 358,
        "protein": 15.2,
        "carbs": 42.8,
        "fat": 16.5,
        "fiber": 12.5,
        "sugar": 8.2,
        "serving_size": "280 g",
        "serving_grams": 280,
        "source": "Meal Planner",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "meal-008",
        "name": "Rajma Chawal (Kidney Bean Rice)",
        "calories": 320,
        "protein": 14.5,
        "carbs": 58.2,
        "fat": 4.8,
        "fiber": 8.5,
        "sugar": 3.5,
        "serving_size": "300 g",
        "serving_grams": 300,
        "source": "Meal Planner",
        "origin": "India",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "meal-009",
        "name": "Tuna Avocado Salad",
        "calories": 285,
        "protein": 22.8,
        "carbs": 12.5,
        "fat": 18.2,
        "fiber": 7.5,
        "sugar": 5.8,
        "serving_size": "220 g",
        "serving_grams": 220,
        "source": "Meal Planner",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "meal-010",
        "name": "Chole Bhature Combo",
        "calories": 485,
        "protein": 16.8,
        "carbs": 65.5,
        "fat": 18.2,
        "fiber": 8.8,
        "sugar": 6.5,
        "serving_size": "250 g",
        "serving_grams": 250,
        "source": "Meal Planner",
        "origin": "India",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "meal-011",
        "name": "Vegetable Stir Fry with Brown Rice",
        "calories": 245,
        "protein": 8.5,
        "carbs": 45.8,
        "fat": 6.2,
        "fiber": 6.8,
        "sugar": 12.5,
        "serving_size": "320 g",
        "serving_grams": 320,
        "source": "Meal Planner",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "meal-012",
        "name": "South Indian Breakfast Platter",
        "calories": 380,
        "protein": 12.5,
        "carbs": 58.5,
        "fat": 12.8,
        "fiber": 5.5,
        "sugar": 8.2,
        "serving_size": "300 g",
        "serving_grams": 300,
        "source": "Meal Planner",
        "origin": "India",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "meal-013",
        "name": "Grilled Chicken Caesar Salad",
        "calories": 365,
        "protein": 28.5,
        "carbs": 15.2,
        "fat": 22.8,
        "fiber": 4.2,
        "sugar": 6.8,
        "serving_size": "280 g",
        "serving_grams": 280,
        "source": "Meal Planner",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "meal-014",
        "name": "Aloo Gobi with Jeera Rice",
        "calories": 285,
        "protein": 7.8,
        "carbs": 52.5,
        "fat": 6.5,
        "fiber": 5.8,
        "sugar": 8.2,
        "serving_size": "300 g",
        "serving_grams": 300,
        "source": "Meal Planner",
        "origin": "India",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "meal-015",
        "name": "Protein Power Bowl",
        "calories": 425,
        "protein": 35.2,
        "carbs": 28.5,
        "fat": 22.8,
        "fiber": 8.5,
        "sugar": 5.2,
        "serving_size": "350 g",
        "serving_grams": 350,
        "source": "Meal Planner",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    }
]

# Save the meal database
with open('meal.json', 'w', encoding='utf-8') as f:
    json.dump(meal_data, f, ensure_ascii=False, indent=2)

print(f"✅ Created meal.json with {len(meal_data)} sample meals")