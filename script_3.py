# Create the main foods.json database
import json

foods_data = [
    # Fruits
    {
        "id": "f-001",
        "name": "Apple, raw (with skin)",
        "calories": 52,
        "protein": 0.26,
        "carbs": 13.81,
        "fat": 0.17,
        "fiber": 2.4,
        "sugar": 10.4,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-002",
        "name": "Banana, ripe",
        "calories": 89,
        "protein": 1.09,
        "carbs": 22.84,
        "fat": 0.33,
        "fiber": 2.6,
        "sugar": 12.23,
        "serving_size": "100 g",
        "serving_grams": 118,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-003",
        "name": "Orange, fresh",
        "calories": 47,
        "protein": 0.94,
        "carbs": 11.75,
        "fat": 0.12,
        "fiber": 2.4,
        "sugar": 9.35,
        "serving_size": "100 g",
        "serving_grams": 150,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-004",
        "name": "Mango, ripe",
        "calories": 60,
        "protein": 0.82,
        "carbs": 15.0,
        "fat": 0.38,
        "fiber": 1.6,
        "sugar": 13.7,
        "serving_size": "100 g",
        "serving_grams": 200,
        "source": "Local",
        "origin": "India",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-005",
        "name": "Grapes, red",
        "calories": 62,
        "protein": 0.63,
        "carbs": 16.0,
        "fat": 0.16,
        "fiber": 0.9,
        "sugar": 15.48,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-006",
        "name": "Papaya, ripe",
        "calories": 43,
        "protein": 0.47,
        "carbs": 10.82,
        "fat": 0.26,
        "fiber": 1.7,
        "sugar": 7.82,
        "serving_size": "100 g",
        "serving_grams": 150,
        "source": "Local",
        "origin": "India",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-007",
        "name": "Pomegranate seeds",
        "calories": 83,
        "protein": 1.67,
        "carbs": 18.7,
        "fat": 1.17,
        "fiber": 4.0,
        "sugar": 13.67,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-008",
        "name": "Guava, fresh",
        "calories": 68,
        "protein": 2.55,
        "carbs": 14.32,
        "fat": 0.95,
        "fiber": 5.4,
        "sugar": 8.92,
        "serving_size": "100 g",
        "serving_grams": 120,
        "source": "Local",
        "origin": "India",
        "last_updated": "2025-08-01",
        "label_url": ""
    },

    # Vegetables
    {
        "id": "f-009",
        "name": "Spinach, raw",
        "calories": 23,
        "protein": 2.86,
        "carbs": 3.63,
        "fat": 0.39,
        "fiber": 2.2,
        "sugar": 0.42,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-010",
        "name": "Broccoli, raw",
        "calories": 34,
        "protein": 2.82,
        "carbs": 6.64,
        "fat": 0.37,
        "fiber": 2.6,
        "sugar": 1.55,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-011",
        "name": "Tomato, fresh",
        "calories": 18,
        "protein": 0.88,
        "carbs": 3.89,
        "fat": 0.2,
        "fiber": 1.2,
        "sugar": 2.63,
        "serving_size": "100 g",
        "serving_grams": 125,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-012",
        "name": "Carrot, raw",
        "calories": 41,
        "protein": 0.93,
        "carbs": 9.58,
        "fat": 0.24,
        "fiber": 2.8,
        "sugar": 4.74,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-013",
        "name": "Onion, raw",
        "calories": 40,
        "protein": 1.1,
        "carbs": 9.34,
        "fat": 0.1,
        "fiber": 1.7,
        "sugar": 4.24,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-014",
        "name": "Potato, raw",
        "calories": 77,
        "protein": 2.05,
        "carbs": 17.49,
        "fat": 0.09,
        "fiber": 2.1,
        "sugar": 0.78,
        "serving_size": "100 g",
        "serving_grams": 150,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-015",
        "name": "Cauliflower, raw",
        "calories": 25,
        "protein": 1.92,
        "carbs": 4.97,
        "fat": 0.28,
        "fiber": 2.0,
        "sugar": 1.91,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-016",
        "name": "Bell pepper, red",
        "calories": 31,
        "protein": 1.0,
        "carbs": 7.31,
        "fat": 0.3,
        "fiber": 2.5,
        "sugar": 4.2,
        "serving_size": "100 g",
        "serving_grams": 120,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-017",
        "name": "Okra (Bhindi), raw",
        "calories": 33,
        "protein": 1.93,
        "carbs": 7.45,
        "fat": 0.19,
        "fiber": 3.2,
        "sugar": 1.48,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "India",
        "last_updated": "2025-08-01",
        "label_url": ""
    },

    # Grains & Cereals
    {
        "id": "f-018",
        "name": "Basmati rice, raw",
        "calories": 365,
        "protein": 7.1,
        "carbs": 78.0,
        "fat": 0.9,
        "fiber": 1.3,
        "sugar": 0.12,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "India",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-019",
        "name": "Whole wheat flour",
        "calories": 340,
        "protein": 11.6,
        "carbs": 69.4,
        "fat": 1.7,
        "fiber": 11.2,
        "sugar": 0.41,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-020",
        "name": "Oats, rolled",
        "calories": 389,
        "protein": 16.89,
        "carbs": 66.27,
        "fat": 6.9,
        "fiber": 10.6,
        "sugar": 0.99,
        "serving_size": "100 g",
        "serving_grams": 40,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-021",
        "name": "Quinoa, cooked",
        "calories": 120,
        "protein": 4.4,
        "carbs": 21.8,
        "fat": 1.9,
        "fiber": 2.8,
        "sugar": 0.87,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-022",
        "name": "Brown bread slice",
        "calories": 247,
        "protein": 13.0,
        "carbs": 41.0,
        "fat": 4.2,
        "fiber": 6.0,
        "sugar": 5.7,
        "serving_size": "100 g",
        "serving_grams": 30,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },

    # Dairy Products
    {
        "id": "f-023",
        "name": "Milk, whole (3.25% fat)",
        "calories": 61,
        "protein": 3.15,
        "carbs": 4.8,
        "fat": 3.25,
        "fiber": 0,
        "sugar": 5.26,
        "serving_size": "100 ml",
        "serving_grams": 250,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-024",
        "name": "Greek yogurt, plain",
        "calories": 59,
        "protein": 10.19,
        "carbs": 3.6,
        "fat": 0.39,
        "fiber": 0,
        "sugar": 3.56,
        "serving_size": "100 g",
        "serving_grams": 170,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-025",
        "name": "Paneer (Indian cottage cheese)",
        "calories": 265,
        "protein": 18.3,
        "carbs": 1.2,
        "fat": 20.8,
        "fiber": 0,
        "sugar": 2.6,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "India",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-026",
        "name": "Cheddar cheese",
        "calories": 403,
        "protein": 24.9,
        "carbs": 1.3,
        "fat": 33.1,
        "fiber": 0,
        "sugar": 0.5,
        "serving_size": "100 g",
        "serving_grams": 30,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-027",
        "name": "Butter, salted",
        "calories": 717,
        "protein": 0.85,
        "carbs": 0.06,
        "fat": 81.11,
        "fiber": 0,
        "sugar": 0.06,
        "serving_size": "100 g",
        "serving_grams": 10,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },

    # Dry Fruits & Nuts
    {
        "id": "f-028",
        "name": "Almonds, raw",
        "calories": 579,
        "protein": 21.15,
        "carbs": 21.55,
        "fat": 49.93,
        "fiber": 12.5,
        "sugar": 4.35,
        "serving_size": "100 g",
        "serving_grams": 30,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-029",
        "name": "Walnuts, raw",
        "calories": 654,
        "protein": 15.23,
        "carbs": 13.71,
        "fat": 65.21,
        "fiber": 6.7,
        "sugar": 2.61,
        "serving_size": "100 g",
        "serving_grams": 30,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-030",
        "name": "Cashews, raw",
        "calories": 553,
        "protein": 18.22,
        "carbs": 30.19,
        "fat": 43.85,
        "fiber": 3.3,
        "sugar": 5.91,
        "serving_size": "100 g",
        "serving_grams": 30,
        "source": "Local",
        "origin": "India",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-031",
        "name": "Dates, Medjool",
        "calories": 277,
        "protein": 1.81,
        "carbs": 75.03,
        "fat": 0.15,
        "fiber": 6.7,
        "sugar": 66.47,
        "serving_size": "100 g",
        "serving_grams": 24,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-032",
        "name": "Raisins, seedless",
        "calories": 299,
        "protein": 3.07,
        "carbs": 79.18,
        "fat": 0.46,
        "fiber": 3.7,
        "sugar": 59.19,
        "serving_size": "100 g",
        "serving_grams": 30,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },

    # Beverages
    {
        "id": "f-033",
        "name": "Green tea, brewed",
        "calories": 1,
        "protein": 0.22,
        "carbs": 0,
        "fat": 0,
        "fiber": 0,
        "sugar": 0,
        "serving_size": "100 ml",
        "serving_grams": 240,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-034",
        "name": "Black coffee, brewed",
        "calories": 2,
        "protein": 0.28,
        "carbs": 0,
        "fat": 0.02,
        "fiber": 0,
        "sugar": 0,
        "serving_size": "100 ml",
        "serving_grams": 240,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-035",
        "name": "Orange juice, fresh",
        "calories": 45,
        "protein": 0.7,
        "carbs": 10.4,
        "fat": 0.2,
        "fiber": 0.2,
        "sugar": 8.1,
        "serving_size": "100 ml",
        "serving_grams": 250,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-036",
        "name": "Coconut water, fresh",
        "calories": 19,
        "protein": 0.72,
        "carbs": 3.71,
        "fat": 0.2,
        "fiber": 1.1,
        "sugar": 2.61,
        "serving_size": "100 ml",
        "serving_grams": 240,
        "source": "Local",
        "origin": "India",
        "last_updated": "2025-08-01",
        "label_url": ""
    },

    # Proteins
    {
        "id": "f-037",
        "name": "Chicken breast, skinless, raw",
        "calories": 165,
        "protein": 31.0,
        "carbs": 0,
        "fat": 3.6,
        "fiber": 0,
        "sugar": 0,
        "serving_size": "100 g",
        "serving_grams": 120,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-038",
        "name": "Salmon, raw",
        "calories": 208,
        "protein": 25.44,
        "carbs": 0,
        "fat": 12.35,
        "fiber": 0,
        "sugar": 0,
        "serving_size": "100 g",
        "serving_grams": 150,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-039",
        "name": "Egg, whole, raw",
        "calories": 155,
        "protein": 13.35,
        "carbs": 1.12,
        "fat": 10.6,
        "fiber": 0,
        "sugar": 1.12,
        "serving_size": "100 g",
        "serving_grams": 50,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-040",
        "name": "Tuna, raw",
        "calories": 144,
        "protein": 23.33,
        "carbs": 0,
        "fat": 4.9,
        "fiber": 0,
        "sugar": 0,
        "serving_size": "100 g",
        "serving_grams": 120,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },

    # Legumes & Pulses
    {
        "id": "f-041",
        "name": "Lentils, red, cooked",
        "calories": 116,
        "protein": 9.02,
        "carbs": 20.13,
        "fat": 0.38,
        "fiber": 7.9,
        "sugar": 1.8,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "India",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-042",
        "name": "Chickpeas (Garbanzo), cooked",
        "calories": 164,
        "protein": 8.86,
        "carbs": 27.42,
        "fat": 2.59,
        "fiber": 7.6,
        "sugar": 4.8,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-043",
        "name": "Black beans, cooked",
        "calories": 132,
        "protein": 8.86,
        "carbs": 23.71,
        "fat": 0.54,
        "fiber": 8.7,
        "sugar": 0.32,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-044",
        "name": "Kidney beans (Rajma), cooked",
        "calories": 127,
        "protein": 8.67,
        "carbs": 22.8,
        "fat": 0.5,
        "fiber": 6.4,
        "sugar": 0.32,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "India",
        "last_updated": "2025-08-01",
        "label_url": ""
    },

    # More Indian foods
    {
        "id": "f-045",
        "name": "Roti (Chapati), whole wheat",
        "calories": 297,
        "protein": 11.61,
        "carbs": 55.81,
        "fat": 4.65,
        "fiber": 8.1,
        "sugar": 2.58,
        "serving_size": "100 g",
        "serving_grams": 40,
        "source": "Local",
        "origin": "India",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-046",
        "name": "Dal (Mixed lentils), cooked",
        "calories": 108,
        "protein": 7.6,
        "carbs": 16.3,
        "fat": 0.8,
        "fiber": 4.8,
        "sugar": 1.4,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "India",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-047",
        "name": "Basmati rice, cooked",
        "calories": 121,
        "protein": 2.7,
        "carbs": 25.0,
        "fat": 0.3,
        "fiber": 0.4,
        "sugar": 0.1,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "India",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-048",
        "name": "Ghee (Clarified butter)",
        "calories": 876,
        "protein": 0.28,
        "carbs": 0.05,
        "fat": 99.48,
        "fiber": 0,
        "sugar": 0.05,
        "serving_size": "100 g",
        "serving_grams": 5,
        "source": "Local",
        "origin": "India",
        "last_updated": "2025-08-01",
        "label_url": ""
    },

    # Additional vegetables
    {
        "id": "f-049",
        "name": "Cucumber, with peel",
        "calories": 15,
        "protein": 0.59,
        "carbs": 3.63,
        "fat": 0.11,
        "fiber": 0.5,
        "sugar": 1.67,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-050",
        "name": "Lettuce, iceberg",
        "calories": 14,
        "protein": 0.9,
        "carbs": 2.97,
        "fat": 0.14,
        "fiber": 1.2,
        "sugar": 1.97,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },

    # More grains and alternatives
    {
        "id": "f-051",
        "name": "Quinoa, raw",
        "calories": 368,
        "protein": 14.12,
        "carbs": 64.16,
        "fat": 6.07,
        "fiber": 7.0,
        "sugar": 4.57,
        "serving_size": "100 g",
        "serving_grams": 50,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-052",
        "name": "Barley, pearl, cooked",
        "calories": 123,
        "protein": 2.26,
        "carbs": 28.22,
        "fat": 0.44,
        "fiber": 3.8,
        "sugar": 0.56,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },

    # More fruits
    {
        "id": "f-053",
        "name": "Pineapple, fresh",
        "calories": 50,
        "protein": 0.54,
        "carbs": 13.12,
        "fat": 0.12,
        "fiber": 1.4,
        "sugar": 9.85,
        "serving_size": "100 g",
        "serving_grams": 150,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-054",
        "name": "Watermelon, fresh",
        "calories": 30,
        "protein": 0.61,
        "carbs": 7.55,
        "fat": 0.15,
        "fiber": 0.4,
        "sugar": 6.2,
        "serving_size": "100 g",
        "serving_grams": 200,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },

    # Oils and fats
    {
        "id": "f-055",
        "name": "Olive oil, extra virgin",
        "calories": 884,
        "protein": 0,
        "carbs": 0,
        "fat": 100.0,
        "fiber": 0,
        "sugar": 0,
        "serving_size": "100 ml",
        "serving_grams": 15,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-056",
        "name": "Coconut oil",
        "calories": 862,
        "protein": 0,
        "carbs": 0,
        "fat": 99.06,
        "fiber": 0,
        "sugar": 0,
        "serving_size": "100 ml",
        "serving_grams": 15,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },

    # Seeds
    {
        "id": "f-057",
        "name": "Chia seeds",
        "calories": 486,
        "protein": 16.54,
        "carbs": 42.12,
        "fat": 30.74,
        "fiber": 34.4,
        "sugar": 1.39,
        "serving_size": "100 g",
        "serving_grams": 15,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-058",
        "name": "Sunflower seeds",
        "calories": 584,
        "protein": 20.78,
        "carbs": 20.0,
        "fat": 51.46,
        "fiber": 8.6,
        "sugar": 2.62,
        "serving_size": "100 g",
        "serving_grams": 30,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },

    # More vegetables (Indian)
    {
        "id": "f-059",
        "name": "Eggplant (Brinjal), raw",
        "calories": 25,
        "protein": 0.98,
        "carbs": 5.88,
        "fat": 0.18,
        "fiber": 3.0,
        "sugar": 3.53,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "India",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-060",
        "name": "Bottle gourd (Lauki), raw",
        "calories": 14,
        "protein": 0.62,
        "carbs": 3.39,
        "fat": 0.02,
        "fiber": 0.5,
        "sugar": 2.5,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "India",
        "last_updated": "2025-08-01",
        "label_url": ""
    },

    # Spices and herbs (common in Indian cooking)
    {
        "id": "f-061",
        "name": "Turmeric powder",
        "calories": 354,
        "protein": 7.83,
        "carbs": 64.93,
        "fat": 9.88,
        "fiber": 21.1,
        "sugar": 3.21,
        "serving_size": "100 g",
        "serving_grams": 5,
        "source": "Local",
        "origin": "India",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-062",
        "name": "Cumin seeds",
        "calories": 375,
        "protein": 17.81,
        "carbs": 44.24,
        "fat": 22.27,
        "fiber": 10.5,
        "sugar": 2.25,
        "serving_size": "100 g",
        "serving_grams": 5,
        "source": "Local",
        "origin": "India",
        "last_updated": "2025-08-01",
        "label_url": ""
    },

    # More protein sources
    {
        "id": "f-063",
        "name": "Tofu, firm",
        "calories": 70,
        "protein": 8.08,
        "carbs": 1.88,
        "fat": 4.17,
        "fiber": 0.4,
        "sugar": 0.62,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-064",
        "name": "Cottage cheese, low fat",
        "calories": 72,
        "protein": 12.39,
        "carbs": 2.68,
        "fat": 1.02,
        "fiber": 0,
        "sugar": 2.94,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },

    # Sweet potatoes and root vegetables
    {
        "id": "f-065",
        "name": "Sweet potato, raw",
        "calories": 86,
        "protein": 1.57,
        "carbs": 20.12,
        "fat": 0.05,
        "fiber": 3.0,
        "sugar": 4.18,
        "serving_size": "100 g",
        "serving_grams": 130,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },

    # Mushrooms
    {
        "id": "f-066",
        "name": "Mushrooms, white button",
        "calories": 22,
        "protein": 3.09,
        "carbs": 3.26,
        "fat": 0.34,
        "fiber": 1.0,
        "sugar": 1.98,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },

    # Berries
    {
        "id": "f-067",
        "name": "Strawberries, fresh",
        "calories": 32,
        "protein": 0.67,
        "carbs": 7.68,
        "fat": 0.3,
        "fiber": 2.0,
        "sugar": 4.89,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-068",
        "name": "Blueberries, fresh",
        "calories": 57,
        "protein": 0.74,
        "carbs": 14.49,
        "fat": 0.33,
        "fiber": 2.4,
        "sugar": 10.0,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },

    # Indian snacks and packaged foods
    {
        "id": "f-069",
        "name": "Samosa, vegetable (medium)",
        "calories": 308,
        "protein": 5.4,
        "carbs": 32.0,
        "fat": 17.8,
        "fiber": 2.8,
        "sugar": 2.1,
        "serving_size": "100 g",
        "serving_grams": 65,
        "source": "FSSAI",
        "origin": "India",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-070",
        "name": "Idli, plain (steamed rice cake)",
        "calories": 158,
        "protein": 4.6,
        "carbs": 30.8,
        "fat": 1.2,
        "fiber": 1.5,
        "sugar": 1.8,
        "serving_size": "100 g",
        "serving_grams": 40,
        "source": "Local",
        "origin": "India",
        "last_updated": "2025-08-01",
        "label_url": ""
    },

    # More packaged Indian beverages will go in localFSSAI.json
    # Additional whole foods
    {
        "id": "f-071",
        "name": "Green peas, fresh",
        "calories": 81,
        "protein": 5.42,
        "carbs": 14.45,
        "fat": 0.4,
        "fiber": 5.7,
        "sugar": 5.67,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-072",
        "name": "Corn kernels, sweet",
        "calories": 86,
        "protein": 3.27,
        "carbs": 18.7,
        "fat": 1.35,
        "fiber": 2.0,
        "sugar": 6.26,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },

    # More fish
    {
        "id": "f-073",
        "name": "Mackerel, raw",
        "calories": 205,
        "protein": 18.6,
        "carbs": 0,
        "fat": 13.89,
        "fiber": 0,
        "sugar": 0,
        "serving_size": "100 g",
        "serving_grams": 120,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-074",
        "name": "Sardines, raw",
        "calories": 208,
        "protein": 24.62,
        "carbs": 0,
        "fat": 11.45,
        "fiber": 0,
        "sugar": 0,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },

    # More Indian vegetables
    {
        "id": "f-075",
        "name": "Bitter gourd (Karela), raw",
        "calories": 17,
        "protein": 1.0,
        "carbs": 3.7,
        "fat": 0.17,
        "fiber": 2.8,
        "sugar": 1.95,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "India",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-076",
        "name": "Ridge gourd (Tori), raw",
        "calories": 20,
        "protein": 1.2,
        "carbs": 4.35,
        "fat": 0.2,
        "fiber": 1.1,
        "sugar": 2.0,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "India",
        "last_updated": "2025-08-01",
        "label_url": ""
    },

    # More nuts and seeds
    {
        "id": "f-077",
        "name": "Pistachios, raw",
        "calories": 560,
        "protein": 20.16,
        "carbs": 27.17,
        "fat": 45.32,
        "fiber": 10.6,
        "sugar": 7.66,
        "serving_size": "100 g",
        "serving_grams": 30,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-078",
        "name": "Pumpkin seeds",
        "calories": 559,
        "protein": 30.23,
        "carbs": 10.71,
        "fat": 49.05,
        "fiber": 6.0,
        "sugar": 1.4,
        "serving_size": "100 g",
        "serving_grams": 30,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },

    # Fermented foods
    {
        "id": "f-079",
        "name": "Kimchi",
        "calories": 15,
        "protein": 1.1,
        "carbs": 2.4,
        "fat": 0.5,
        "fiber": 1.6,
        "sugar": 1.1,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "Global",
        "last_updated": "2025-08-01",
        "label_url": ""
    },

    # More Indian staples
    {
        "id": "f-080",
        "name": "Poha (Flattened rice), dry",
        "calories": 325,
        "protein": 6.6,
        "carbs": 76.9,
        "fat": 0.6,
        "fiber": 0.2,
        "sugar": 0.12,
        "serving_size": "100 g",
        "serving_grams": 50,
        "source": "Local",
        "origin": "India",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-081",
        "name": "Upma (Semolina dish), cooked",
        "calories": 168,
        "protein": 3.8,
        "carbs": 28.6,
        "fat": 4.2,
        "fiber": 1.8,
        "sugar": 1.2,
        "serving_size": "100 g",
        "serving_grams": 100,
        "source": "Local",
        "origin": "India",
        "last_updated": "2025-08-01",
        "label_url": ""
    },
    {
        "id": "f-082",
        "name": "Dosa, plain (fermented rice crepe)",
        "calories": 168,
        "protein": 4.1,
        "carbs": 32.8,
        "fat": 2.6,
        "fiber": 1.2,
        "sugar": 0.8,
        "serving_size": "100 g",
        "serving_grams": 60,
        "source": "Local",
        "origin": "India",
        "last_updated": "2025-08-01",
        "label_url": ""
    }
]

# Save the foods database
with open('foods.json', 'w', encoding='utf-8') as f:
    json.dump(foods_data, f, ensure_ascii=False, indent=2)

print(f"✅ Created foods.json with {len(foods_data)} food items")